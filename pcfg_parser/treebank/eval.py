...
class TreeOperations:
    ...
    def _fringe(self, node):
        if len(node) == 2:
            ...
            return self._fringe(node[1]) + self._fringe(node[2])
    ...
    def _well_formed(self, node):
        if len(node) not in [2, 3]:
            raise ParseError(
                "Ill-formed tree:  %d-ary rule, only binary or unary allowed %s" % (
                len(node), node))

        if not isinstance(node[0], str):
            raise ParseError(
                "Ill-formed tree: non-terminal not a string %s." % (node[0]))

        if len(node) == 2:
            if not isinstance(node[1], str):
                raise ParseError(
                    "Ill-formed tree: unary rule does not produce a string %s." % (
                    node[1]))
        elif len(node) == 3:
            if isinstance(node[1], str):
                raise ParseError(
                    "Ill-formed tree: binary rule produces a string %s." % (
                    node[1]))
...
class ParseEvaluator:
    ...
    def compute_fscore(self, key_trees, predicted_trees):
        for trees in zip(key_trees, predicted_trees):
            ...
            if len(f1) != len(f2):
                raise ParseError(
                    "Sentence length does not match. Gold sentence length %d, "
                    "test sentence length %d. Sentence '%s'" % (
                    len(f1), len(f2), " ".join(f1)))

            for gold, test in zip(f1, f2):
                if test != "_RARE_" and gold != test:
                    raise ParseError(
                        "Tree words do not match. "
                        "Gold sentence '%s', test sentence '%s'." % (
                        " ".join(f1), " ".join(f2)))
